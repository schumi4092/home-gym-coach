...
### v0.5.0
- 筆記感知教練建議：解析訓練筆記中的狀態關鍵字（疲勞 / 睡眠、痠痛、喘、未完成），下次建議據此調整；並把你上次寫的筆記回顯給你看
- 每組獨立重量：記錄各組實際重量（標出最重組與「各組不同」），上次摘要可逐組顯示
- 分數步進：依目前重量自動推斷 kg 步進（0.125–5），動作卡片與課表 / 歷史編輯器共用同一組步進選單，所有重量統一正規化
- 自動帶入重量：開新訓練自動帶上次的工作重量；完成訓練後同步更新課表的起始重量
- 月曆翻頁：可前後切換月份並一鍵回到本月，修正跨月的今天 / 未來標記；找不到對應課表的紀錄改蓋中性印章不再變空白
- 歷史紀錄改派課表：可把一筆紀錄重新指定到其他課表（修正月曆歸屬）；組列新增熱身（WU）切換鈕
- 計時器修正：自動起跳 / 重置改為延後更新，避免在 render 中 setState
- 連接埠改為 `127.0.0.1:8780`（8765–8864 為 Windows 保留範圍會綁定失敗），Node 不在 PATH 時 fallback 用 8088，綁定失敗會自動重試
